    ...
    public function dispatch() {
        ...
        switch ($step) {
            ...
            case 1 :
                ...
                //_e( 'Error with handle_upload!', 'hw_cmt_import_export' );
            ...
            case 2 :
                ...
                if ($file) {
                    ...
                    <table id="import-progress" class="widefat_importer widefat">     <thead>
                            <tr>
                                <th class="status">&nbsp;</th>
                                <th class="row"><?php _e('Row', 'hw_cmt_import_export'); ?></th>
                                <th><?php _e('ID', 'hw_cmt_import_export'); ?></th>
                                <th><?php _e('Comment Link', 'hw_cmt_import_export'); ?></th>
                                <th class="reason"><?php _e('Status Msg', 'hw_cmt_import_export'); ?></th>
                            </tr>     </thead>
                        <tfoot>        
                        ...
                        var crosssell_skus = [];
                                ...
                                $('#import-progress tbody').append('<tr id="row-' + i + '" class="error"><td class="status" colspan="5">' + results.error + '</td></tr>');
                                    ...
                                    $('#import-progress tbody').append('<tr id="row-' + i + '" class="' + row['status'] + '"><td><mark class="result" title="' + row['status'] + '">' + row['post_id'] + '</mark></td><td class="row">' + i + '</td><td>' + row['post_id'] + '</td><td> <a href="' + row['comment_link'] + '" target="_blank" title="Comment:  ' + row['cmd_title'] + '" >Comment :' + row['post_id'] + '</a>  </td><td class="reason">' + row['reason'] + '</td></tr>');
                                        ...
                                        $('#import-progress tbody').append('<tr class="error"><td class="status" colspan="5">' +    '<?php _e('AJAX Error', 'hw_cmt_import_export'); ?>' + '</td></tr>');
                                            }
...
// Remainder
                    ...
                    ?>
                            ...
                            type: 'POST',
                                    url: ajaxurl,
                                    data: { action: "product_comments_csv_import_regenerate_thumbnail", id: id },
                                    success: function(response) {
                                    if (response !== Object(response) || (typeof response.success === "undefined" && typeof response.error === "undefined")) {
                                    response = new Object;
                                    response.success = false;
                                    response.error = "<?php printf(esc_js(__('The resize request was abnormally terminated (ID %s). This is likely due to the image exceeding available memory or some other type of fatal error.', 'hw_cmt_import_export')), '" + id + "'); ?>";
                                    }

                                    regen_count ++;
                                    $('#import-progress tbody .regenerating .progress').css('width', ((regen_count / attachments.length) * 100) + '%').html(regen_count + ' / ' + attachments.length + ' <?php echo esc_js(__('thumbnails regenerated', 'hw_cmt_import_export')); ?>');
                                         if (! response.success) {
                    ...
                    echo '<p class="error">' . __('Error finding uploaded file!', 'hw_cmt_import_export') . '</p>';
                }
                break;
            case 3 :
            ...
            case 4 :
                ...
                _e('Step 1...', 'hw_cmt_import_export') . ' ';

                wp_defer_term_counting(true);
                wp_defer_comment_counting(true);

                _e('Step 2...', 'hw_cmt_import_export') . ' ';
                ...
                _e('Finalizing...', 'hw_cmt_import_export') . ' ';


                // SUCCESS
                _e('Finished. Import complete.', 'hw_cmt_import_export');
    ...
    public function import_options() {
        ...
        if ($mapping_from_db && is_array($mapping_from_db) && $this->profile !=='' && count($mapping_from_db) == 2 && empty($_GET['clearmapping'])) {
            ...
            echo '<h3>' . __('Columns are pre-selected using the Mapping file: "<b style="color:gray">' . $this->profile . '</b>".  <a href="' . $reset_action . '"> Delete</a> this mapping file.', 'hw_cmt_import_export') . '</h3>';
    ...
    public function import() {
        ...
        if (function_exists('WC')) {
            ...
            $this->log->add('csv-import', __('Processing product comments.', 'hw_cmt_import_export'));
        } else {
       ...
       if (function_exists('WC')) {
            $this->log->add('csv-import', __('Finished processing product comments.', 'hw_cmt_import_export'));
        } else {
            // $this->log = new WP_Logger();
         }
    ...
    public function import_start($file, $mapping, $start_pos, $end_pos, $eval_field) {
        ...
        if(function_exists('WC'))
        {
         ...
         $wp_memory = size_format(woocommerce_let_to_num(WP_MEMORY_LIMIT));
            ...
            $this->log->add('csv-import', __('Parsing product comments CSV.', 'hw_cmt_import_export'));
        }
        else
        {
    ...
    if(function_exists('WC'))
    {
        $this->log->add('csv-import', __('Finished parsing product comments CSV.', 'hw_cmt_import_export'));
    }
...
public function product_id_not_exists($id,$cmd_type) {
        ...
        if ($cmd_type === 'comment') {
            ...
            $query = "SELECT ID FROM $wpdb->posts WHERE ID = %d AND post_type='product' AND comment_status='open'";
            $posts_that_exist = $wpdb->get_col($wpdb->prepare($query,$id));
            if (!$posts_that_exist) {
                return true;
            }
    ...
    public function handle_upload() {
        ...
        if (empty($_POST['file_url'])) {
            ...
            if (isset($file['error'])) {
                echo '<p><strong>' . __('Sorry, there has been an error.', 'hw_cmt_import_export') . '</strong><br />';
            ...
            if (file_exists(ABSPATH . $_POST['file_url'])) {
                ...
                echo '<p><strong>' . __('Sorry, there has been an error.', 'hw_cmt_import_export') . '</strong></p>';
                return false;
            }
        }
    ...
    public function process_product_comments($post) {
        ...
        if ($post['comment_type'] != 'woodiscuz') {
            $cmd_type = 'comment';
            $product_post='This Post Does Not Exits';
        } else {
            $cmd_type = $post['comment_type'];
             $product_post='This Product Does Not Exits';
        }
        ...
        if (!empty($processing_product_id) && isset($this->processed_posts[$processing_product_id])) {
            $this->add_import_result('skipped', __('Product comment already processed', 'hw_cmt_import_export'), $processing_product_id, $comment_txt);
            if(function_exists('WC'))
            {
            $this->log->add('csv-import', __('> Post ID already processed. Skipping.', 'hw_cmt_import_export'), true);
            }
            unset($post);
            return;
        }
        ...
        if (!empty($post['post_status']) && $post['post_status'] == 'auto-draft') {
            $this->add_import_result('skipped', __('Skipping auto-draft', 'hw_cmt_import_export'), $processing_product_id, $comment_txt);
           if(function_exists('WC'))
           {
            $this->log->add('csv-import', __('> Skipping auto-draft.', 'hw_cmt_import_export'), true);
           }
            unset($post);
            return;
        }
        ...
        if (!$merging) {
            ...
            if ($is_post_exist_in_db) {
                ...
                $usr_msg = 'This Comment ID Already Exits';
                $this->add_import_result('skipped', __($usr_msg, 'hw_cmt_import_export'), $processing_product_id, $comment_txt);
            if(function_exists('WC'))
            {
                $this->log->add('csv-import', sprintf(__('> &#8220;%s&#8221;' . $usr_msg, 'hw_cmt_import_export'), esc_html($processing_product_title)), true);
            }
                unset($post);
                return;
            }
        ...
        if ($merging && !empty($is_post_exist_in_db)) {
            ...
            if(function_exists('WC'))
            {
            $this->log->add('csv-import', sprintf(__('> Merging post ID %s.', 'hw_cmt_import_export'), $post_id), true);
            }
            if (!empty($post['comment_post_ID'])) {
           ...
           if(function_exists('WC'))
            {
            $this->log->add('csv-import', sprintf(__('> Inserting %s', 'hw_cmt_import_export'), esc_html($processing_product_id)), true);
            }
            ...
            $postdata = array(
                ...
                'comment_author_email' => $post['comment_author_email'], 'comment_author_email' => $post['comment_author_email'],
            ...
            if(function_exists('WC'))
            {
            $this->log->add('csv-import', sprintf(__($_SESSION['new_id'][$post['comment_alter_id']] . 'hi'), esc_html($processing_product_title)));
            }
            if (is_wp_error($post_id)) {

                $this->add_import_result('failed', __('Failed to import product comment', 'hw_cmt_import_export'), $processing_product_id);
             if(function_exists('WC'))
             {
                $this->log->add('csv-import', sprintf(__('Failed to import product comment &#8220;%s&#8221;', 'hw_cmt_import_export'), esc_html($processing_product_title)));
             }
                unset($post);
                return;
            } else {
               if(function_exists('WC'))
               {
                $this->log->add('csv-import', sprintf(__('> Inserted - post ID is %s.', 'hw_cmt_import_export'), $post_id));
               }
                }
        }
        unset($postdata);
        ...
        if ($merging) {
           ...
           if(function_exists('WC'))
           {
            $this->log->add('csv-import', sprintf(__('> Finished merging post ID %s.', 'hw_cmt_import_export'), $post_id));
           }
            ...
            if(function_exists('WC'))
            {
            $this->log->add('csv-import', sprintf(__('> Finished importing post ID %s.', 'hw_cmt_import_export'), $post_id));
            }
            }
        unset($post);
    }
    ...
    public function fetch_remote_file($url, $post) {
        ...
        if (0 == $filesize) {
            @unlink($upload['file']);
            unset($upload);
            return new WP_Error('import_file_error', __('Zero size file downloaded', 'hw_cmt_import_export'));
        }
    ...
    public function header() {
        ...
        echo '<h2>' . ( empty($_GET['merge']) ? __('Import', 'hw_cmt_import_export') : __('Merge WordPress Comments', 'hw_cmt_import_export') ) . '</h2>';
    }
